...
```json
{ "error": { "code": "negative_balance_not_acceptable", "message": "Human-readable detail." } }
```
...
A household is auto-created server-side on a parent's **first** Firebase sign-in (SDS §7.1 footnote). We call it Auto-Registration.
...
* `403 owner_only` — caller is not the household creator.
...
`initialPin` is returned **only here** (shown to the parent once). Logged: `ChildCreated` (SDS §8).
...
* Then SignalR `OnBalanceUpdated` push to group `child_{id}` so the child's open dashboard re-renders in the new denomination (SDS §4/§7.2).
...
`currencyKey` is the ledger-row snapshot taken at insert time (SDS §4). Then SignalR `OnBalanceUpdated` push to group `child_{id}` (SDS §4).
...
The endpoint surface above matches SDS §7.1 exactly (all former "derived endpoints" are now first-class SDS endpoints). The following **behavioral details** are specified here and still lack an explicit SDS statement:

* Error status mapping (`422` for negative balance, `423` for lockouts, `403` for IP ban) — SDS §3.3/§4 define the logic but not HTTP statuses.
* `PUT /household/parents/me/pin` request shape (`currentPin` verification before change).
* `initialPin` returned only in the `POST /household/children` response.
* SignalR `OnBalanceUpdated` also fired after a currency change (§5.3) so the child dashboard re-renders in the new denomination.
