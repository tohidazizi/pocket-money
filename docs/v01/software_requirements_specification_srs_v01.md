...
* **FR-C2 (Persistent Child Session):**
  ...
  * Child can manually logout from the account.
    ...
    * Does logout require Pocket-Money Account ID next time? Yes — unless the device's local child-history (UI SRS) still lists the profile, in which case re-login is PIN-only from the chooser.
...
* **FR-S1 (Session Separation):**
  ...
  * On a shared device the parent logs out before handing the device to a child (UI SRS, ChildrenHistory flow).
